...
from portiq_server.serializers.linkedin_serializers import GetLinkedinDataSerializer, PutLinkedinDataSerializer
...
class LinkeinViewSetDetails(viewsets.ViewSet):
    ...
    def update_linkedin_data(self, request):
        ...
        user.first_name = user.first_name if not data.get("first_name") else data.get("first_name")
        user.last_name = user.last_name if not data.get("last_name") else data.get("last_name")
        user.email = user.email if not data.get("email") else data.get("email")
        user.phone_number = user.phone_number if not data.get("phone_number") else data.get("phone_number")
        user.image_url = user.image_url if not data.get("image_url") else data.get("image_url")
        user.save()
